...
# GUARDO EL STOCK EN EL ARCHIVO
def guardar_stock(productos):
    with open('stock.json', 'w') as file:
        json.dump(productos, file)
        
# PARA CARGAR EL STOCK, LE AGREGO TRY EN CASO DE QUE NO ENCUENTRE EL ARCHIVO. 
def cargar_stock():
    try:
...
#ACA PARA GUARDAR LA VENTAS
def guardar_ventas(ventas):
    with open('ventas.json', 'w') as file:
        json.dump(ventas, file)
        
# LO MISMO QUE EL STOCK PERO PARA VENTAS. 
def cargar_ventas():
    try:
